...
void CircleParticle::update()
{
  if( state == EXPANSION )
  {
    // if we are here it means particle is expanding.
    ...
    {
      // if we are here it means the particle is expanding but it
      ...
      for(int j=0; j<nPalline; j++)
      {
        if( j != parentId )
        {
          ...
          // We reach a ball when shockwave is hitting the ball center.
  ...
  {
    // particle is on its IDLE state so do nothing
  }
