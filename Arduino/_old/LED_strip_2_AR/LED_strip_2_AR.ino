...
void setup() 
{
  ...
  for(i=0; i<NPADS; i++)
  {
    blocks[i].ar.init(FADEIN_TIME, FADEOUT_TIME);
    blocks[i].tail = i*PPP;
...
void loop() 
{
  ...
  {
    blocks[i].ar.update();
    blocks[i].rawAmp = blocks[i].ar.getY();
    ...
    if(DEBUG) {Serial.print( blocks[i].ar.getState() );Serial.print("\t");}
    for(j=0; j<PPP; j++)
    {
