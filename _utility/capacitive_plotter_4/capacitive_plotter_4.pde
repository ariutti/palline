...
// this is a delayed version of the reset in order for the graphs
// deltas not to be confused by temporary transitional spikes on the MPR.
// In other words, we set the bestDelta to zero only after one second the 
// MPR has been resetted.
void delayedResetThread() {
  println("delayed reset thread: waiting...");
  delay(1000);
  println("Reset!");
  for(int i=0; i<NGRAPHS; i++) {
    graphs[i].reset();
